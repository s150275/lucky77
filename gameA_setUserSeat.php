...
/*
if(($_SESSION['Member_ID']!="Member2010040001")&&($_SESSION['Member_ID']!="Member2011050002")&&($_SESSION['Member_ID']!="Member2011080001"))
{
...
if($_SESSION['zone_code'] == 2)
{
	$tmp_CheckSQL_Banker3 = "SELECT * FROM BetGodnine WHERE BetGodnine_Bingo_Period = '{$array_BingoPeriod['NowBingo']}' AND BetGodnine_Type = '3' AND BetGodnine_On = '1'" ;
	$Count_Banker3 = func_DatabaseGet( $tmp_CheckSQL_Banker3 , "COUNT" , "" ) ;		// 取得資料庫資料
	if($Count_Banker3>=1)
	{
		echo "-1,長莊有倍率已有投注，不可跨區投注，下期才可繼續投注";
		exit;
	}
}
//長莊有倍率與長莊無倍率同期不可跨區投注
if($_SESSION['zone_code'] == 3)
{
	$tmp_CheckSQL_Banker2 = "SELECT * FROM BetGodnine WHERE BetGodnine_Bingo_Period = '{$array_BingoPeriod['NowBingo']}' AND BetGodnine_Type = '2' AND BetGodnine_On = '1'" ;
	$Count_Banker2 = func_DatabaseGet( $tmp_CheckSQL_Banker2 , "COUNT" , "" ) ;		// 取得資料庫資料
	if($Count_Banker2>=1)
	{
		echo "-1,長莊無倍率已有投注，不可跨區投注，下期才可繼續投注";
		exit;
	}
}
...
if( $_SESSION['zone_code'] == 2 OR $_SESSION['zone_code'] == 3)
{
	...
	else
	{
		...
		$Table_number = $array_Bankset_Info['Banker_Banker_Id'];
		
		$seat_number1 = explode(",",$array_Bankset_Info['Banker_Banker_Seats'])[0];
		$seat_number2 = explode(",",$array_Bankset_Info['Banker_Banker_Seats'])[1];
...
{// START 此座位是否已有人選取
	...
	if( $_SESSION['zone_code'] == 2 OR $_SESSION['zone_code'] == 3 )
	{
		...
		echo $array_BankerList_Info['Now_Banker_Seat']."<br>";
	}
	*/
