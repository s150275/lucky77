	...
	methods:
	{
		...
		getGameInfo()
		{
			...
			.then(response =>
			{
				...
				// 上期長莊有倍區金額
				document.getElementById("WinLost_Area3_Points").innerHTML = this.GameInfo.WinLost_Area3_Points ;
				// 上期輸贏總計
				...
				// 設定派彩點數CSS
				if( Number(this.GameInfo.WinLost_Area3_Points) > 0 )
				{	$("#WinLost_Area3_Points").css({'color':"#64e664"}) ;	}
				else if( Number(this.GameInfo.WinLost_Area3_Points) < 0 )
				{	$("#WinLost_Area3_Points").css({'color':"#F00"}) ;	}
				else
				{	$("#WinLost_Area3_Points").css({'color':"#FCEE21"}) ;	}
