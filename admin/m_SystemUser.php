...
$MAIN_FILE_NAME		= "m_SystemUser.php" ;			// 設定本程式的檔名
...
// 設定查詢欄位
$array_Search_Field['SystemUser_Name'] = "姓名" ;
$array_Search_Field['SystemUser_Login_Name'] = "帳號" ;
$array_Search_Field['SystemUser_ID'] = "員工ID" ;
$array_Search_Field['SystemUser_Email'] = "Email" ;
$array_Search_Field['SystemUser_Birthday'] = "生日" ;
...
// 性別
$array_SystemUser_Sex['M'] = "男" ;
$array_SystemUser_Sex['F'] = "女" ;
...
function getPara()
{
	...
	// 參數五 : 變數格式(INT)
	//不用可以設成"-"即可
	$ARRAY_POST_GET_PARA1[] = "SystemUser_Name||*||ADD||MOD||-" ;
	$ARRAY_POST_GET_PARA1[] = "SystemUser_Email||*||ADD||MOD||-" ;		// Email
	$ARRAY_POST_GET_PARA1[] = "SystemUser_Birthday||*||ADD||MOD||-" ;	// 生日
	$ARRAY_POST_GET_PARA1[] = "SystemUser_Sex||*||ADD||MOD||-" ;		// 性別
	$ARRAY_POST_GET_PARA1[] = "SystemUser_Content||*||ADD||MOD||-" ;	// 備註

	$ARRAY_POST_GET_PARA1[] = "SystemUser_Login_Name||*||ADD||-||-" ;	// 備註
	$ARRAY_POST_GET_PARA1[] = "SystemUser_Login_Passwd1||*||-||-||-" ;	// 備註
	$ARRAY_POST_GET_PARA1[] = "SystemUser_Login_Passwd2||*||-||-||-" ;	// 備註

//	$ARRAY_POST_GET_PARA1[] = "SystemUser_PutTop||*||ADD||MOD||INT" ;	// 置頂
	$ARRAY_POST_GET_PARA1[] = "SystemUser_On||*||-||MOD||INT" ;
    ...
    <div class="row">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
			...
			echo "    <label>名稱</label>\n";
			if ( $subFUNCT != "SHOW" )
			{	echo "<input type=\"text\" name=\"SystemUser_Name\" id=\"SystemUser_Name\" class=\"form-control\" value=\"" . $subLIST['SystemUser_Name'] . "\" required>\n";	}
			else
			{	echo $subLIST['SystemUser_Name'] . "\n";	}
			echo "</div>\n";
			
			echo "<div class=\"form-group\">\n";
			echo "    <label>帳號</label>\n";
			if ( $subFUNCT != "SHOW" AND !($subFUNCT == "MODOK") )
			{	echo "    <input type=\"text\" name=\"SystemUser_Login_Name\" id=\"SystemUser_Login_Name\" class=\"form-control\" value=\"" . $subLIST['SystemUser_Login_Name'] . "\" required>\n";	}
			else
			{
				echo $subLIST['SystemUser_Login_Name'] . "\n";
				echo "    <input name=\"SystemUser_Login_Name\" type=\"hidden\" id=\"SystemUser_Login_Name\" value=\"" . $subLIST['SystemUser_Login_Name'] . "\">\n";
			}
			echo "</div>\n";

			if ( $subFUNCT == "ADDOK" OR $subFUNCT == "MODOK" )
			{
				echo "<div class=\"form-group\">\n";
				echo "	<label>登入密碼</label>\n";
				echo "	<input type=\"password\" name=\"SystemUser_Login_Passwd1\" id=\"SystemUser_Login_Passwd1\" class=\"form-control\" value=\"\">\n";
				echo "</div>\n";
				echo "<div class=\"form-group\">\n";
				echo "	<label>再次輸入密碼</label>\n";
				echo "	<input type=\"password\" name=\"SystemUser_Login_Passwd2\" id=\"SystemUser_Login_Passwd2\" class=\"form-control\" value=\"\">\n";
				echo "</div>\n";
			}
			...
			if ( $subFUNCT != "SHOW" )
			{	echo "<input type=\"text\" name=\"SystemUser_Email\" id=\"SystemUser_Email\" class=\"form-control\" value=\"" . $subLIST['SystemUser_Email'] . "\">\n";	}
			else
			{	echo $subLIST['SystemUser_Email'] . "\n";	}
			echo "</div>\n";
			
			echo "<div class=\"form-group\">\n";
			echo "    <label>生日</label>\n";
			if ( $subFUNCT != "SHOW" )
			{	echo "<input type=\"date\" name=\"SystemUser_Birthday\" id=\"SystemUser_Birthday\" class=\"form-control\" value=\"" . $subLIST['SystemUser_Birthday'] . "\">\n";	}
			else
			{	echo $subLIST['SystemUser_Birthday'] . "\n";	}
			echo "</div>\n";
			...
			if ( $subFUNCT != "SHOW" )
			{
				echo "<select name=\"SystemUser_Sex\" class=\"form-control\">\n";
				foreach( $array_SystemUser_Sex as $key => $value )
				{
					if ( $key == $subLIST['SystemUser_Sex'] )
			...
			{	echo $Array_City[$subLIST['SystemUser_Sex']] . "\n";	}
			echo "</div>\n";
			
			echo "<div class=\"form-group\">\n";
			echo "    <label>備註</label><br>\n";
			if ( $subFUNCT != "SHOW" )
			{	echo "<textarea id=\"SystemUser_Content\" name=\"SystemUser_Content\" style=\"width:80%;height:200px;\">" . $subLIST['SystemUser_Content'] . "</textarea>\n";	}
			else
			{	echo nl2br($subLIST['SystemUser_Content']) . "\n";	}
			echo "</div>\n";
	...
	font-size:18px;
		...
		<div id="page-wrapper">
			...
			else
			{
				...
				{	$countSQL = "SELECT * FROM $MAIN_DATABASE_NAME WHERE $SEARCH_FIELD Like '%" . (int)$SEARCH_KEY . "%' OR $SEARCH_FIELD LIKE '$SEARCH_KEY%' ORDER BY $MAIN_CHECK_FIELD DESC";	}
				elseif ( $SEARCH_FIELD AND $SEARCH_KEY)
				{	$countSQL = "SELECT * FROM $MAIN_DATABASE_NAME WHERE $SEARCH_FIELD LIKE '%$SEARCH_KEY%' ORDER BY $MAIN_CHECK_FIELD DESC";	}
				else
				{	$countSQL = "SELECT * FROM $MAIN_DATABASE_NAME ORDER BY $MAIN_CHECK_FIELD DESC";	}
				//echo "$countSQL<br>" ;
				...
				{		  
					...
					echo "    <td><a href=\"$MAIN_FILE_NAME?Funct=SHOW&ID=" . $LIST[$MAIN_CHECK_FIELD] . "\">" . $LIST["SystemUser_Name"] . "</a></td>\n";

					echo "    <td>"
					. $LIST["SystemUser_Login_Name"]
					. "</td>\n";
