...
★配合程式
	◆Manage_Business.php				主程式
		查詢欄位設定
		表格設定
	◆Manage_Business_Ajax_Del.php		刪除資料
		設定資料庫名稱
	◆Manage_Business_Ajax_List.php	找出資料列表
		查詢SQL設定
	◆anage_Business_Form.php			秀出燈箱資料
		設定要執行的SQL資料

★副程式(func_bchiayi.php)-每個程式都要建一個副程式
	◆bchiayi_getBusiness_List() ;		取得店家資料列表
		設定要回傳的表格資料
*/

$MAIN_PROGRAM_TITLE	= "代理人操作記錄" ;					// 設定程式的TITLE文字
$MAIN_BASE_ADDRESS	= "../" ;						// 設定檔案和首頁的相對位置
$MAIN_FILE_NAME_ADDRESS	= "admin/" ;				// 設定本程式所在的路徑
$MAIN_DATABASE_NAME	= "Agent" ;						// 設定本程式所使用的基本資料庫
$MAIN_FILE_NAME		= "m_LogInfo_Agent.php" ;				// 設定本程式的檔名
...
if ( $Funct == "Log" AND $ID )
{// 秀出某區間的所有資料
	$array_LogAdmin_Info = func_DatabaseGet( "LogAdmin" , "*" , array("id_LogAdmin"=>"$ID ") ) ;		// 取得資料庫資料
	//echo "<p>Log內容</p>" ;print_r($array_LogAdmin_Info);echo "<br>" ;
	
	$arrayConvLog = func_AnalysisLogInfo( $array_LogAdmin_Info['LogAdmin_Msg'] ) ;	// 分析LOG資料
...
{// 秀出目前最新10筆記錄資料
	$SQL = "SELECT * FROM  LogAdmin WHERE LogAdmin_Database = 'System_Log' ORDER BY id_LogAdmin DESC LIMIT 0 , 10" ;
	//echo $SQL . "<br>" ; 
	...
	if ( mysqli_num_rows($QUERY) )
	{
		...
		while ($LIST = mysqli_fetch_assoc($QUERY))
		{
			...
			echo "        <a href='$MAIN_FILE_NAME?Funct=Log&ID={$LIST['id_LogAdmin']}' class='btn btn-primary'>記錄日期 : {$LIST['LogAdmin_Start_DT']} - {$LIST['LogAdmin_End_DT']}</a><br>" ;
			echo "        </div>\n" ;
