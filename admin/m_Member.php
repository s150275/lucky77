...
$MAIN_FILE_NAME		= "m_Member.php" ;			// 設定本程式的檔名
...
// 設定查詢欄位
$array_Search_Field['Member_Name'] = "代理人名稱" ;
$array_Search_Field['Member_ID'] = "代理人ID" ;
$array_Search_Field['Member_Login_Name'] = "登入帳號" ;

// 設定子類別選項
$array_subType[] = "開啟||SEARCH_FIELD=Member_On&SEARCH_KEY=1" ;
$array_subType[] = "停用||SEARCH_FIELD=Member_On&SEARCH_KEY=0" ;

// 性別
$array_Member_Sex['M'] = "男" ;
$array_Member_Sex['F'] = "女" ;
...
function getPara()
{
	...
	// 參數五 : 變數格式(INT)
	//不用可以設成"-"即可
	$ARRAY_POST_GET_PARA1[] = "Member_Name||*||ADD||MOD||-" ;

	$ARRAY_POST_GET_PARA1[] = "Member_Login_Name1||*||-||-||-" ;			// 帳號姓
	$ARRAY_POST_GET_PARA1[] = "Member_Login_Name2||*||-||-||-" ;			// 帳號名
	$ARRAY_POST_GET_PARA1[] = "Member_Login_Passwd1||*||-||-||-" ;		// 密碼
	$ARRAY_POST_GET_PARA1[] = "Member_Login_Passwd2||*||-||-||-" ;		// 密碼
	
	$ARRAY_POST_GET_PARA1[] = "Member_Level||*||ADD||MOD||-" ;			// 帳號權限
	$ARRAY_POST_GET_PARA1[] = "Member_Email||*||ADD||MOD||-" ;			// Email
	$ARRAY_POST_GET_PARA1[] = "Member_Birthday||*||ADD||MOD||-" ;		// 生日
	$ARRAY_POST_GET_PARA1[] = "Member_Sex||*||ADD||MOD||-" ;				// 性別
	$ARRAY_POST_GET_PARA1[] = "Member_Mobile||*||ADD||MOD||-" ;			// 手機
	$ARRAY_POST_GET_PARA1[] = "Member_City||*||ADD||MOD||-" ;			// 縣市
	$ARRAY_POST_GET_PARA1[] = "Member_Money||*||ADD||MOD||-" ;			// 金額
	$ARRAY_POST_GET_PARA1[] = "Member_Backwater||*||ADD||MOD||-" ;		// 返水比例
	$ARRAY_POST_GET_PARA1[] = "Member_Offline_Share||*||ADD||MOD||-" ;	// 下線分成比
	$ARRAY_POST_GET_PARA1[] = "Member_Content||*||ADD||MOD||-" ;			// 備註
	$ARRAY_POST_GET_PARA1[] = "Member_Sort||*||ADD||MOD||-" ;			// 排序

	//$ARRAY_POST_GET_PARA1[] = "Member_PutTop||*||ADD||MOD||INT" ;	// 置頂
	$ARRAY_POST_GET_PARA1[] = "Member_On||*||-||MOD||INT" ;
...
function showForm( $subLIST , $subFUNCT )
{
	...
	if ( $subFUNCT != "SHOW" )
	{	echo "<input type=\"text\" name=\"Member_Name\" id=\"Member_Name\" class=\"form-control\" value=\"" . $subLIST['Member_Name'] . "\" required>\n";	}
	else
	{	echo $subLIST['Member_Name'] . "\n";	}
	echo "</div>\n";
	...
	if ( $subFUNCT == "ADDOK" )
	{
		...
		echo "<input type=\"text\" name=\"Member_Login_Name1\" id=\"Member_Login_Name1\" class=\"form-control\" value=\"\" minlength='2' maxlength='2' required style='width:50px;display:inline-block;'>\n";
		echo "<input type=\"text\" name=\"Member_Login_Name2\" id=\"Member_Login_Name2\" class=\"form-control\" value=\"\" minlength='6' maxlength='18' required style='width:60%;display:inline-block;'>\n";
		echo "</div>\n";
	}
	else
	{
		echo "<div class=\"form-group\">\n";
		echo "	<label>登入帳號</label><br>\n";
		echo "	{$subLIST['Member_Login_Name']}\n";
		echo "<input type=\"hidden\" name=\"Member_Login_Name\" id=\"Member_Login_Name\" value=\"{$subLIST['Member_Login_Name']}\">\n";
		echo "</div>\n";
	}
	...
	echo "	<input type=\"password\" name=\"Member_Login_Passwd1\" id=\"Member_Login_Passwd1\" class=\"form-control\" value=\"\">\n";
	echo "</div>\n";
	...
	echo "	<input type=\"password\" name=\"Member_Login_Passwd2\" id=\"Member_Login_Passwd2\" class=\"form-control\" value=\"\">\n";
	echo "</div>\n";
	...
	if ( $subFUNCT != "SHOW" )
	{	echo "<input type=\"text\" name=\"Member_Boss_Name\" id=\"Member_Boss_Name\" class=\"form-control\" value=\"" . $subLIST['Member_Boss_Name'] . "\">\n";	}
	else
	{	echo $subLIST['Member_Boss_Name'] . "\n";	}
	echo "</div>\n";
	...
	if ( $subFUNCT != "SHOW")
	{
		echo "<select name=\"Member_City\" class=\"form-control\">\n";
		foreach( $Array_City as $key => $value )
		{
			if ( $key == $subLIST['Member_City'] )
	...
	{	echo $Array_City[$subLIST['Member_City']] . "\n";	}
	echo "</div>\n";

	echo "<div class=\"form-group\">\n";
	echo "    <label>地址</label>\n";
	if ( $subFUNCT != "SHOW" )
	{	echo "<input type=\"text\" name=\"Member_Address\" id=\"Member_Address\" class=\"form-control\" value=\"" . $subLIST['Member_Address'] . "\">\n";	}
	else
	{	echo $subLIST['Member_Address'] . "\n";	}
	echo "</div>\n";

	echo "<div class=\"form-group\">\n";
	echo "    <label>電話</label>\n";
	if ( $subFUNCT != "SHOW" )
	{	echo "<input type=\"text\" name=\"Member_Tel\" id=\"Member_Tel\" class=\"form-control\" value=\"" . $subLIST['Member_Tel'] . "\">\n";	}
	else
	{	echo $subLIST['Member_Tel'] . "\n";	}
	echo "</div>\n";

	echo "<div class=\"form-group\">\n";
	echo "    <label>手機</label>\n";
	if ( $subFUNCT != "SHOW" )
	{	echo "<input type=\"text\" name=\"Member_Mobile\" id=\"Member_Mobile\" class=\"form-control\" value=\"" . $subLIST['Member_Mobile'] . "\">\n";	}
	else
	{	echo $subLIST['Member_Mobile'] . "\n";	}
	echo "</div>\n";

	if ( $subFUNCT == "SHOW" ){ $tmpShow = "readonly";	}
	echo "<div class=\"form-group\">\n";
	echo "    <label>介紹</label>\n";
	echo "    <textarea name=\"Member_Content\" id=\"Member_Content\" class=\"form-control\" rows=\"10\" $tmpShow>" . $subLIST['Member_Content'] . "</textarea>\n";
	echo "</div>\n";
...
//            echo "</div>\n";
	...
	if ( $subFUNCT != "SHOW")
	{
		echo "<input type=\"radio\" name=\"Member_On\" id=\"Member_On1\" value=\"1\"" . checksCheckBox($subLIST['Member_On'], 1) . "><label for=\"Member_On1\">" . $array_On[1] . "</label>\n";
		echo "<input type=\"radio\" name=\"Member_On\" id=\"Member_On0\" value=\"0\"" . checksCheckBox($subLIST['Member_On'], 0) . "><label for=\"Member_On0\">" . $array_On[0] . "</label>\n";
	}
	else
	{	echo $array_On[(int)$subLIST['Member_On']] . "\n";	}
	echo "</div>\n";
...
elseif ( $Funct == "ADDOK" )
{
	...
	$Member_Login_Name1 = strtoupper($Member_Login_Name1) ;	// 轉成大寫
	// 帳號姓名的長度
	if( strlen($Member_Login_Name1) != 2 )
	{	$errMsg = "帳號姓名的長度不為2,請重新設定..." ;	}
	// 查詢帳號是否已存在
	else if ( CheckFieldExist( "Member" , "Member_Login_Title" , "$Member_Login_Name1" ) )
	{	$errMsg = "帳號組別已存在,請重新設定..." ;	}
	else
	{
		// 設定帳號
		$tmpSQL .= " Member_Login_Name = '$Member_Login_Name1$Member_Login_Name2' , " ;
		$tmpSQL .= " Member_Login_Title = '$Member_Login_Name1' , " ;
	}

	if ( $Member_Login_Passwd1 == "" )
	{	$errMsg = "密碼要設定..." ;	}
	elseif ( $Member_Login_Passwd1 != $Member_Login_Passwd2 )
	{	$errMsg = "兩個密碼不相同,請重新設定..." ;	}
	else
	{
		if ( strlen($Member_Login_Passwd1) < 6 OR strlen($Member_Login_Passwd1) > 18 )
		{	$errMsg = "密碼長度為6-18..." ;	}
		else
		{	$tmpSQL .= " Member_Login_Passwd = '" . crypt("$Member_Login_Passwd" , $Member_Login_Name1.$Member_Login_Name2) . "' , " ;	}
	}
...
elseif ( $Funct == "MODOK" )
{
	...
	else
	{
		...
		{	$tmpSQL .= " Member_Login_Passwd = '" . crypt($Member_Login_Passwd1 , $Member_Login_Name). "' , " ;	}
	}
...
elseif ( $Funct == "ON" OR $Funct == "DOWN")
{
	...
	{
		$Member_on = "1" ;
		$Member_on_str = $array_On[$Member_on] ;
	}
	else
	{
		$Member_on = "0" ;
	...
	{
		alertgo( "$Member_on_str 資料修改完成" , $MAIN_FILE_NAME ) ;
	}
	else
	{
		$errMsg = "$Member_on_str 修改資料失敗!!" ;
	}
}
//~@_@~// E N D 上下架資料 ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
//~@_@~// START 列表資料 ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
else
{
	...
	{		  
		...
		echo "	<td><a href=\"m_Member.php?FUNCT=LOGIN&ID=" . $LIST['id_Member'] . "\">" . $LIST['Member_ID'] . "</a>
		 [<a href=\"" . $MAIN_BASE_ADDRESS . "login.php?Funct=LOGIN&Login_Name=" . $LIST['Member_Login_Name'] . "\" target=\"_blank\">Ｖ</a>]</td>\n";
		echo "	<td>\n";
		echo "	" . $LIST['Member_Name'] . "<br>\n" ;
		echo "	</td>\n";
		echo "	<td>\n";
		echo "	" . $LIST['Member_Login_Name'] . "<br>\n" ;
		echo "	</td>\n";
		echo "	<td>\n";
		echo "	" . $LIST['Member_Backwater'] . "%<br>\n" ;
		echo "	</td>\n";
		echo "	<td>\n";
		echo "	下級代理 : <span class=\"" . $array_Create_CSS[(int)$LIST["Member_Open_Offline"]] . "\">" . $array_Create[(int)$LIST['Member_Open_Offline']] . "</span><br>\n" ;
		echo "	下線會員 : <span class=\"" . $array_Create_CSS[(int)$LIST["Member_Open_Offline"]] . "\">" . $array_Create[(int)$LIST['Member_Open_Member']] . "</span><br>\n" ;
		echo "	</td>\n";
