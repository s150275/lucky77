...
//				}
			...
			{// START 不是相同房間-寫入莊家資料
				...
				// 求出該期當莊資料
				// 如果為長莊區-----------------------------
				if( $LIST['BetGodnine_Type'] == 2 OR $LIST['BetGodnine_Type'] == 3 )
				{// 長莊區-當莊資料
				if( $Old_BetGodnine_Type == 1 AND $LIST['BetGodnine_Type'] == 2 )
				{	$tmp_Has_Data = 1 ;	}
				if( $Old_BetGodnine_Type == 1 AND $LIST['BetGodnine_Type'] == 3 )
				{	$tmp_Has_Data = 1 ;	}
					
					//修改長莊位置輪轉後輸贏資料1031
					//取出該期的莊主選位值切換資料並將json解回Array
					...
					/*{
						...
						if((!$array_BankerList_Info['Now_Banker_Seat'])||(!$array_BankerList_Info['Now_Period'])||($array_BankerList_Info['Next_Period']<$array_Bingo_Info['Bingo_Period']))
						{			
							$Table_number = 19;
						}
						else
						{
							...
							for($tb=0;$tb<count($array_BankerList_Info['Banker_Info']);$tb++)
							{
								...
								if($array_BankerList_Info['Banker_Info'][$tb]['Banker']==$array_BankerList_Info['last_Banker_Seat'])
								{
					...
					/*		$Table_number = $LIST['Banker_Table'];
					...
					// 取出該期的莊主選位的計算值-內定20
					$tmp_Banker_Calculate = $array_Bingo_Godnine_Calculate[$Table_number];
					// 莊家選取的桌號
					$tmp_Banker_Table = $Table_number+1 ;
					// 設定長莊-手續費退水Key
...
//$Old_BetGodnine_Bingo_Period = $LIST['BetGodnine_Bingo_Period'] ;
			...
			else
			{
				...
				if( $LIST['BetGodnine_Type'] == 3 )
				{
					// 輪莊有倍數區
					$tmp_Multiple = $array_Bingo_Godnine_Multiple[$tmp_Banker_Table-1] ;
					$array_Banker_Odds['Banker'] = $tmp_Multiple ;
					$array_Banker_Odds['User'] = $array_Bingo_Godnine_Multiple[$LIST['BetGodnine_Table']-1] ;
					$tmp_Backwater_Ratio = 0.05 ;	// 手續費比例 : 所嬴籌碼 5%

					$tmp_Log = "設定長莊有倍數區的輸贏賠率:\n" ;
					$tmp_Log .= "莊家輸贏賠率({$array_Bingo_Draw_Order_Num[$tmp_Banker_Table-1]} / $tmp_Banker_Calculate) : {$array_Banker_Odds['Banker']}\n" ;
					$tmp_Log .= "閒家輸贏賠率({$array_Bingo_Draw_Order_Num[$LIST['BetGodnine_Table']-1]} / $tmp_Bet_Calculate) : {$array_Banker_Odds['User']}\n" ;
					$tmp_Log .= "手續費比例 : $tmp_Backwater_Ratio\n" ;
				}
			...
			// 取得所有閒家上線代理人資料
			$array_AgentList = GodNine_getAgentList( $LIST['BetGodnine_Agent_ID'] , "A" , $LIST['BetGodnine_Type'] ) ;		// 取得所有上線代理人資料
			if ( $tmpShowMsg )	// 秀出除錯訊息 ██████████
			{
			...
			// 如果為輸莊區
			if( $LIST['BetGodnine_Type'] == 1 OR $LIST['BetGodnine_Type'] == 3)
			{
				// 取得所有莊家資料
				$array_Banker_Member = GodNine_getMemberInfo( $array_Banker_Info['Banker_Set_ID'] ) ;
				// 取得莊家所有上線代理人資料
				$array_Banker_AgentList = GodNine_getAgentList( $array_Banker_Member['Member_Father_ID'] , "A" , $LIST['BetGodnine_Type'] ) ;		// 取得所有上線代理人資料
				if ( $tmpShowMsg )	// 秀出除錯訊息 ██████████
				{
			...
			{// 莊家贏
				...
				// 莊家輸贏總金額(莊家贏)
				if( $LIST['BetGodnine_Type'] == 2 )// 長莊區 有倍與無倍 = 莊家輸贏金額 * (100-佔成)/100
				...
				if( $LIST['BetGodnine_Type'] == 2 OR $LIST['BetGodnine_Type'] == 3)
				{
					// 會員下注未拆帳總金額(閒家輸) = 會員下注總金額 + 會員下注返水金額
					$tmp_Member_WinLost_AllMoney = $tmp_Member_WinLost_Money + $tmp_Member_Backwater_Money;
					// 會員下注報帳金額(閒家輸) = 會員下注未拆帳總金額 * 佔成數
					$tmp_Member_Report_Money = $tmp_Member_WinLost_AllMoney * (100 - $tmp_Member_Proportion ) / 100 ;
				}
				else
				{
					$tmp_Member_WinLost_AllMoney = $tmp_Member_WinLost_Money;
			...
			{// 閒家贏
				...
				*/
				...
				if( $LIST['BetGodnine_Type'] == 2 OR $LIST['BetGodnine_Type'] == 3)
				{
					// 會員下注報帳金額(閒家贏) = 會員下注總金額 + 會員下注返水金額
					$tmp_Member_WinLost_AllMoney = $tmp_Member_WinLost_Money + $tmp_Member_Backwater_Money;
					// 會員下注報帳金額(閒家贏) = 會員下注未拆帳總金額 * 佔成數
					$tmp_Member_Report_Money = $tmp_Member_WinLost_AllMoney * (100 - $tmp_Member_Proportion ) / 100 ;
				}
				else
				{
				...
				// 會員下注輸贏金額(閒家贏)
				if( $LIST['BetGodnine_Type'] == 2 OR $LIST['BetGodnine_Type'] == 3 )// 長莊區 = 會員下注輸贏金額(閒家贏) * (100-佔成)/100
			...
			foreach( $array_AgentList["I"] as $key => $value )
			{
				...
				*/
				...
				// 長莊有倍率區計算方式上線報帳金額 = 上線輸贏金額 + 上線返水金額 + 有佔成
				if ($LIST['BetGodnine_Type'] == 3){
					$array_AllMoney[$key] = $array_AllMoney[$key] + $array_Backwater_Money[$key];
					$array_Reported_Money[$key] = $array_AllMoney[$key] * (100 - $array_AgentList['S'][$key]) / 100;
				}// 長莊無倍率區計算方式上線報帳金額 = 上線輸贏金額 + 上線返水金額 + 無佔成
				elseif ($LIST['BetGodnine_Type'] == 2){
					$array_AllMoney[$key] = $array_AllMoney[$key] + $array_Backwater_Money[$key];
					$array_Reported_Money[$key] = $array_AllMoney[$key] * (100 - $array_AgentList['S'][$key]) / 100;
				}
				else {
			...
			//$arrayField_WinLost['Banker_Table'] = $Table_number; //莊家該期桌號
...
function GodNine_getMember_LastWinLostPoints( $sub_Member_ID = "" )
{
	...
	*/
	...
	$array_LastWinLostPoints['WinLost_Area3_Points'] = 0 ;		// 上期長莊區有倍率金額
	...
	// 求出閒家上期長莊有倍率派彩點數
	$tmpSQL_BetGodnine3 = "SELECT sum(BetGodnine_Member_WinLost_Money) as BetGodnine_Member_WinLost_Money_Total FROM BetGodnine WHERE BetGodnine_Bingo_Period = '{$array_BingoPeriod['LastBingo']}' AND BetGodnine_Member_ID = '$sub_Member_ID' AND BetGodnine_Type = '3' AND BetGodnine_On = '1'" ;				// 找出某欄位的總合
	$array_BetGodnine_Member_Info3 = func_DatabaseGet( $tmpSQL_BetGodnine3 , "SQL" , "" ) ;		// 取得資料庫資料

	$array_LastWinLostPoints['WinLost_Area3_Points'] = (int)$array_BetGodnine_Member_Info3['BetGodnine_Member_WinLost_Money_Total'] ;		// 上期長莊區有倍率金額
	$array_LastWinLostPoints['WinLost_Total_Points'] = (int)($array_LastWinLostPoints['WinLost_Area1_Points'] + $array_LastWinLostPoints['WinLost_Area2_Points'] + $array_LastWinLostPoints['WinLost_Area3_Points']) ;		// 上期輸贏總計

	return $array_LastWinLostPoints ;
}
//~@_@~// END 取得會員上期輸贏點數 ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
//~@_@~// START 取得排莊列表 ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
function GodNine_htmlBankerArea($action="")
{
	global $link;
	/*
	...
	*/
	...
	if( $_SESSION['zone_code'] == 2 OR $_SESSION['zone_code'] == 3 )
	{
		...
		{
			$array_BingoPeriod['NowBingo'] = $array_BingoPeriod['NowBingo'];
		}
		else
		{
			$array_BingoPeriod['NowBingo'] = $array_BingoPeriod['NowBingo']+1;
		}
		...
		{
			$tmp_htmlBankerArea .= "							<p id='N' class='BankerInfo'>下一局(T桌)</p>\n";
		}
		else
		{

			$tmp_htmlBankerArea .= "							<p id='N' class='BankerInfo'>下一局({$array_Bankset_Info['Banker_Banker_Table']})</p>\n";
		}
		...
		if($array_BankerList_Info!="")
		{
			...
			{
				if(empty($array_BankerList_Info['Banker_Info']))
				{
					$tmp_htmlBankerArea = "							<p id='N' class='BankerInfo'>系統</p>\n";
				}
				else
				...
				else
				{
					...
					for($i=0;$i<count($array_BankerList_Info['Banker_Info']);$i++)
					{
						...
						{
							$tmp_htmlBankerArea .= "							<p id='N' class='BankerInfo'>下一局({$array_BankerList_Info['Banker_Info'][$i]['Banker_Name']})</p>\n";
						}
					}
				}
...
function GodNine_getBetGodnineMoney( $subSQL , $subID = "" )
{
	...
	*/
	...
	$array_BetGodnineMoney[3]['BetGodnine_Money'] = 0 ;						// 投注金額
	$array_BetGodnineMoney[3]['BetGodnine_Online_WinLost_Money'] = 0 ;		// 總金額
	$array_BetGodnineMoney[3]['BetGodnine_Online_Backwater_Money'] = 0 ;		// 會員返水
	$array_BetGodnineMoney[3]['BetGodnine_Online_AllMoney'] = 0 ;				// 輸贏金額
	$array_BetGodnineMoney[3]['BetGodnine_Online_Reported_Money'] = 0 ;		// 報帳金額
...
function GodNine_htmlReportInfo( $subType = "Agent" )
{
	...
	*/
	...
	if( $subType == "Agent" )
	{// 代理
		echo "						<td rowspan=\"4\" align=\"center\">代理</td>\n";
		echo "						<td rowspan=\"4\" align=\"center\">{$LIST_Agent['Agent_Name']}</td>\n";
		echo "						<td rowspan=\"4\" align=\"center\">{$LIST_Agent['Agent_Login_Name']}</td>\n";
	}
	else
	{// 會員
		echo "						<td rowspan=\"4\" align=\"center\">會員</td>\n";
		echo "						<td rowspan=\"4\" align=\"center\">{$LIST_Member['Member_Name']}</td>\n";
		echo "						<td rowspan=\"4\" align=\"center\">{$LIST_Member['Member_Login_Name']}</td>\n";
	}
	...
	// 查看明細
	if( $subType == "Agent" )// 代理
	{	echo "						<td rowspan=\"4\" align=\"center\"><a href=\"report.php?AID={$LIST_Agent['id_Agent']}&Report_Start_Date=$Report_Start_Date&Report_End_Date=$Report_End_Date&Report_Start_Hour=$Report_Start_Hour&Report_End_Hour=$Report_Start_Hour\">查看明細</a></td>\n";	}
	else
	{	echo "						<td rowspan=\"4\" align=\"center\"><a href=\"report_detail.php?MID={$LIST_Member['id_Member']}&Report_Start_Date=$Report_Start_Date&Report_End_Date=$Report_End_Date&Report_Start_Hour=$Report_Start_Hour&Report_End_Hour=$Report_Start_Hour\">查看明細</a></td>\n";	}
	...
	//長莊有倍率
	if( $subType == "Agent" )// 代理-
	{	$tmp_report_List3_Para = "report_List.php?Funct=report_List&Agent_ID={$LIST_Agent['id_Agent']}&BetGodnine_Type=3&Report_Start_Date=$Report_Start_Date&Report_Start_Hour=$Report_Start_Hour&Report_End_Date=$Report_End_Date&Report_End_Hour=$Report_End_Hour" ;	}
	else
		//{	$tmp_report_List3_Para = "report_List.php?Funct=report_List&MID=&MID={$LIST_Member['Member_ID']}&BetGodnine_Type=3&Report_Start_Date=$Report_Start_Date&Report_Start_Hour=$Report_Start_Hour&Report_End_Date=$Report_End_Date&Report_End_Hour=$Report_End_Hour" ;	}
	{	$tmp_report_List3_Para = "javascript:;" ;	}
	echo "					<tr class=\"$tmp_TR_CSS\">\n";
	echo "						<td align=\"right\">長莊有倍數區</td>\n";
	// 投注金額
	echo "						<td align=\"right\"><a href='$tmp_report_List3_Para'>" . WinHappy_setMoneyCss( $array_BetGodnineMoney[3]['BetGodnine_Money'] ) . "</a></td>\n";
	// 輸贏金額
	echo "						<td align=\"right\">" . WinHappy_setMoneyCss( $array_BetGodnineMoney[3]['BetGodnine_Online_WinLost_Money'] ) . "</td>\n";
	// 會員返水
	echo "						<td align=\"right\">" . WinHappy_setMoneyCss( $array_BetGodnineMoney[3]['BetGodnine_Online_Backwater_Money'] ) . "</td>\n";
	// 總金額
	echo "						<td align=\"right\">" . WinHappy_setMoneyCss( $array_BetGodnineMoney[3]['BetGodnine_Online_AllMoney'] ) . "</td>\n";

	// 占成/退水
	if( $subType == "Agent" )// 代理
	{	echo "						<td align=\"right\">{$LIST_Agent['Agent_Share3']}%/{$LIST_Agent['Agent_Backwater3']}%</td>\n";	}
	else
	{	echo "						<td align=\"right\">{$array_AgentReport_Info['Agent_Share3']}%/{$array_AgentReport_Info['Agent_Backwater3']}%</td>\n";	}

	// 報帳金額
	echo "						<td align=\"right\">" . WinHappy_setMoneyCss( func_Digital_Carry( $array_BetGodnineMoney[3]['BetGodnine_Online_Reported_Money'] , 2 , "round" ) ) . "</td>\n";
	echo "					</tr>\n";
...
// 代理人相關
//~@_@~// START 取得所有上線代理人資料 ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
function GodNine_getAgentList( $tmp_Agent_ID , $subType = "A" , $BetType = "" )
{
	global $link;
	/*
	...
	*/
	...
	while(1)
	{
		...
		if ( $subType == "A" OR preg_match("/S/i",$subType) )
		{
			...
			{
				// 自己該負責的占成 = 本身占成 - 下層占成
				$array_AgentListS[] = $array_Agent_Info['Agent_Share3'] ;
			}
			else
			{
				// 自己該負責的占成 = 本身占成 - 下層占成
		...
		if ( $subType == "A" OR preg_match("/W2/i",$subType) )
		{
			...
			{
				// 自己該負責的占成 = 本身占成 - 下層占成
				$array_AgentListW2[] = $array_Agent_Info['Agent_Backwater3'] ;
			}
			else
			{
				// 自己該負責的占成 = 本身占成 - 下層占成
