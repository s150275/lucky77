...
var name,point,period,opentime=0,page='index',logining=false,lang='tw';
...
function randBet(p){
	...
	numberAry=[];
	while(numberAry.length!=p)clickBet(Math.floor(Math.random()*80)+1);
}
function clickBet(p){
	if(opentime<1){alert(Lang(336));return;}
	var on=true;
	for(var i=numberAry.length-1;i>-1;i--) if(numberAry[i]==p) numberAry.splice(i,1),i=-1,on=false;
	if(on&&numberAry.length<8) numberAry.push(p);
	chBet();
	sumBet();
}
function chBet(){
	for(var i=1;i<=80;i++) if(document.getElementById('d_'+i)!=null) document.getElementById('d_'+i).className='betB';
	for(var i=0;i<numberAry.length;i++) if(document.getElementById('d_'+numberAry[i])!=null) document.getElementById('d_'+numberAry[i]).className='betC';
	for(var i=1;i<=5;i++){
		if(i>numberAry.length){
			document.getElementById('sum_'+(i+2)).innerHTML='0';
			document.getElementById('vuvuh_'+(i+2)).style.background=i%2==0?'#FFF':'';
			document.getElementById('vuvuh_'+(i+2)).style.color='';
			document.getElementById('mo_'+(i+2)+'_0').value='';
...
function pageLimit(){
	...
	//document.getElementById('code1').innerHTML=html+pageEnd(400);
	getLimit();
}
function toDate(s){
...
function gogotime(){
	...
	if(document.getElementById('opentime')!=null){
		if(opentime==0&&valAry.length!=0)noBet();
...
function sumBet(){
	valAry=[];betpoint=0;
	for(var i=0;i<3;i++) for(var j=0;j<2;j++) sumBetT(i,j);
	for(var i=3;i<8;i++) sumBetT(i,0);/*
...
function enterBet(r){
	...
	if(r)sumBet();
	if(betpoint>point){alert(Lang(344)+'('+commafy(betpoint)+' > '+commafy(point)+')');return;};
	if(betpoint>0){
		enterBeting=true;		
		var b=new creatRequestObj();
...
function vuvuary(i){
	var tnum=[1,2,2,3,3];
	if(i>=3){var h2=[];for(var u=0;u<tnum[i-3];u++)h2.push(document.getElementById('odd_'+i+'_'+u).innerHTML);return h2.join(',');}
}
function sumBetT(i,j){
	var k=parseInt(document.getElementById('mo_'+i+'_'+j).value.split(',').join(''),10);
	if(k>0){		
		if(k<limitAry[0][i+1]){k=0;alert(Lang(348)+' '+commafy(limitAry[0][i+1]));}
		if(k>limitAry[1][i+1]){k=0;alert(Lang(349)+' '+commafy(limitAry[1][i+1]));}
		if(k>point){k=0;alert(Lang(344)+' '+commafy(point));}
		var p=1;if(i>=3) p=parseInt(document.getElementById('sum_'+i).innerHTML,10);k*=p;
		var h=document.getElementById('odd_'+i+'_'+j).innerHTML;
		if(i>=3) h=vuvuary(i);
		if(k>0)valAry.push((i+1)+'@'+(i>=3?numberAry.join(','):j)+'@'+k+'@'+h);
		else document.getElementById('mo_'+i+'_'+j).value=commafy(p==0?0:k/p);
		betpoint+=k;
	}		
}
var rTick=[];
	...
	b.onreadystatechange=function(){
		if(b.readyState==4){
			...
			if(a.indexOf('-')>-1)err(a);
				...
				limitAry=[];
				var v=a.split('@');
				for(var i=0;i<v.length;i++) limitAry[i]=v[i].split(',');
				if(page=='limit')
...
function pagePlayRule(){
	...
	document.getElementById('code1').innerHTML=html+pageEnd(1000);
}
function pageQA(){
	page='QA';
