...
if ( $MAIN_INCLUDE_NAME == "game" )
{
	...
	*/
	//1104修改房間只有一間
	$_SESSION['Room'] = $Room;
	if( $Room >= 1 AND $Room <= 1 )
	{	$_SESSION['Room'] = $Room ;	}
	//else if( empty($_SESSION['Chips']) )
	else if( $_SESSION['Room'] != 1 )
	{	alertgo("選擇房間的相關資料有錯,請重新選擇","money.php");	}
}

if ( $MAIN_INCLUDE_NAME == "room" OR $MAIN_INCLUDE_NAME == "game" )
{
	if( $zone_code == 1 OR $zone_code == 2 OR $zone_code == 3 )
	{	$_SESSION['zone_code'] = $zone_code ;	}
	//else if( empty($_SESSION['Chips']) )
	else if( !($_SESSION['zone_code'] == 1 OR $_SESSION['zone_code'] == 2 OR $_SESSION['zone_code'] == 3) )
