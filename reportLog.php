...
// 財神九仔生
$MAIN_INCLUDE_NAME	= "reportLog" ;				// 主要程式名稱
...
echo "					<input type=\"date\" name='Search_Date' placeholder=\"日期\" value='" . date("Y-m-d") . "' \>\n";
echo "					<a id='Search_Date'>搜尋</a>\n";
echo "				</div>\n";
echo "			</div>\n";
echo "		</div>\n";

echo "		<div id='reportLogArea' class=\"bingoHistory\">\n";
...
?>
<script>
function ajax_htmlOperationRecordList( Funct , Para )
{
	// 設定要傳的POST參數
	var tmp_PostData = {};
	...
	$.ajax({
		type: 'POST',
		url: 'reportLogA_htmlOperationRecordList.php', 
		data: tmp_PostData ,
	})
	.done(function(data){
		data = $.trim(data);
...
$("#Search_Date , #Search_Bingo_Period").click(function() {
	...
	ajax_htmlOperationRecordList( this.id , Para );
})
// 先載入今日資料
ajax_htmlOperationRecordList( "Search_Date" , "ToDay" );
