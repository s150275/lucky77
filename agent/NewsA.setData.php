...
$ARRAY_POST_GET_PARA[] = "News_Title||*" ;	// 消息標題
$ARRAY_POST_GET_PARA[] = "News_On||*" ;		// 消息狀態
...
if ( $Funct == "ADDOK" )
{
	...
	// 錯誤訊息
	$errMsg = "" ;
	$Global_ADDSQL = " News_On = '$News_On'" ;

	// 密碼設定OK
	if ( $errMsg == "" )
	{
		$tmpSQL .= " News_Title = '$News_Title' , " ;	// 佔成比
		$tmpSQL .= " News_Add_DT = '" . $MAIN_NOW_TIME . "' , " ;	// 佔成比
		$tmpSQL .= " News_Mod_DT = '" . $MAIN_NOW_TIME . "' , " ;	// 佔成比
		...
		if(mysqli_query($link , $insertSQL))
		{
			...
			$array_LogInfo['Info'] = "動作:新增最新消息 , 操作者:{$_SESSION['Agent_Name']} , 消息標題:$News_Title , 新增ID:$id" ;	// 操作記錄 備註(可記錄新增會員ID,刪除ID)
...
{
	$arrayField['News_On'] = $News_On ;
	$arrayField['News_Title'] = $News_Title ;
	$arrayField['News_Mod_DT'] = $MAIN_NOW_TIME ;

	$Bol_News = func_DatabaseBase( "News" , "MOD" , $arrayField , " id_News = '$ID'" ) ;						// 資料庫處理
	if( $Bol_News )
	{
		// 加入操作LOG
		$array_LogInfo['Time'] = date("Y-m-d H:i:s") ;					// 操作日期
		...
		$array_LogInfo['Info'] = "動作:修改最新消息 , 操作者:{$_SESSION['Agent_Name']} , 修改ID:$ID" ;	// 操作記錄 備註(可記錄新增會員ID,刪除ID)
		$array_LogInfo['SQL'] = str_replace ("'","’",$insertSQL) ;		// SQL內容(有才需填-只給管理者看)
...
else if ( $Funct == "DELOK" )
{
	...
	if( $ID )
	{
		$Bol_Agent = func_DatabaseBase( "News" , "DEL" , "" , " id_News = '$ID'" ) ;
		...
		$array_LogInfo['Info'] = "動作:刪除最新消息 , 操作者:{$_SESSION['Agent_Name']} , 刪除ID:$ID" ;	// 操作記錄 備註(可記錄新增會員ID,刪除ID)
		$array_LogInfo['SQL'] = str_replace ("'","’",$insertSQL) ;		// SQL內容(有才需填-只給管理者看)
